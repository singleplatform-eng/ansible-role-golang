...
# Todo:
#  - Add support for parsing a .go.sh file in the root of a project (that selects one of the gopath functions)
#  - Verify sourcing behavior
...
{% if go_path is defined %}
{# Set some defaults coming into the script. but allow users to override with any flag.#}
# If user didn't specify anything, try the templated var,
# then default (/usr/local/go)
if [ -e "{{go_path}}" ]; then
    _GOPATH="{{go_path}}"
else
...
# todo: consider merging the fallback_go_settings.sh setup here.
# needs comparison to see which is more "right"
# set_go_path
